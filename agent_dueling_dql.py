...
class DQN(nn.Module):
    ...
    def __init__(self, ep: EnvironmentBounds):
        ...
        # self.hidden_layer2V = nn.Linear(1024, 512)
        # self.hidden_layer2A = nn.Linear(1024, 512)
    ...
    def forward(self, x):
        ...
        print("Hello in Dueling DQN***************************************")
        print(x.shape)
        # x = F.dropout(x, p=0.5, training=self.training)
        # value = F.relu(self.hidden_layer2V(x))
        # adv = F.relu(self.hidden_layer2A(x))
        ...
        # x = F.relu(self.hidden_layer4(x))
        ###############################################
        # print(self.head(adv.view(adv.size(0), -1)))
        # print(self.head(adv))
        print("Net output************")
        print(f'value output ={self.headvalue(value.view(value.size(0),-1)).shape}')
        ...
        # adv = self.headadv(adv.view(x.size(0), -1))
        ...
        # return self.headadv(adv.view(adv.size(0), -1))
        return Qoutput
