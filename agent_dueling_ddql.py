...
class DQN(nn.Module):
    ...
    def __init__(self, ep: EnvironmentBounds):
        ...
        print("linear and output size")
        print(f'Linear input state= {len(model.state_space.dim_sizes)}')
        ...
        self.hidden_layer3V = nn.Linear(512, 128)
        self.hidden_layer3A = nn.Linear(512, 128)
        # self.hidden_layer4 = nn.Linear(128, 64)

        self.headvalue = nn.Linear(128, 1)  # For state value
        self.headadv = nn.Linear(128, output_size)  ## for advantage value

    # Called with either one element to determine next action, or a batch
    # during optimization. Returns tensor([[left0exp,right0exp]...]).
    def forward(self, x):
        x = F.relu(self.hidden_layer1(x))
        print("Hello in Dueling Double DQN***************************************")
        print(x.shape)
        # x = F.dropout(x, p=0.5, training=self.training)
        x = F.relu(self.hidden_layer2(x))
        print(f'L2 = {x.shape}')
        # # x = F.dropout(x, p=0.5, training=self.training)
        value = F.relu(self.hidden_layer3V(x))
        adv = F.relu(self.hidden_layer3A(x))
        print(f'advantage = {adv.shape}')
        print(f'Value = {value.shape}')

        print("Net output************")
        print(f'value output ={self.headvalue(value).shape}')
        # value = self.headvalue(value)
        value = self.headvalue(value.view(value.size(0), -1))
        print(f'The value output ={value.shape}')

        print(self.headadv(adv.view(x.size(0), -1)).shape)
        print(f'adv value output ={self.headadv(adv).shape}')
        print(self.headadv(adv.view(x.size(0), -1)).shape)
        # adv = self.headadv(adv)
        adv = self.headadv(adv.view(adv.size(0), -1))
        print(f'The adv value output ={adv.shape}')

        advAverage = torch.mean(adv, dim=1, keepdim=True)
        print(f'advAverage ={advAverage.shape}')
        advcen = adv - advAverage
        print(f'advcent = {advcen.shape}')

        Qoutput = value + advcen
        print(f'output ={Qoutput.shape}')

        # print(f'adv ={adv}')
        # print(f'value ={value}')
        # print(f'advAverage ={advAverage}')
        # print(f'advcen={advcen}')
        # print(f'output ={Qoutput}')

        return Qoutput


        # # The orginal DQL or DDQL
        # x = F.relu(self.hidden_layer1(x))
        # print(x.shape)
        # # x = F.dropout(x, p=0.5, training=self.training)
        # x = F.relu(self.hidden_layer2(x))
...
class DeepQLearnerPolicy(Learner):
    ...
    def optimize_model(self, norm_clipping=False):
        ...
        print("********************************************************Heyyy:  this is policy outputs for Dueling DDQL")
