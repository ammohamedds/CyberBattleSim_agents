...
class DQN(nn.Module):
    ...
    def __init__(self, ep: EnvironmentBounds):
        ...
        print(len(model.state_space.dim_sizes))
...
class DeepQLearnerPolicy(Learner):
    ...
    def optimize_model(self, norm_clipping=False):
        ...
        ########## Compute online or predicted Q values using Online or policy Network #############
        ...
        print("********************************************************Heyyy:  this is policy outputs")
        print(f'output={output.shape}')
        print(f'state_action_values={state_action_values.shape}')
        ########## Compute Expected Q values using target or Expected Network #############
        ...
        ################ DQN target
        print("################################ This DQN Target Calculation")
        next_state_values = torch.zeros(self.batch_size, device=device)
        next_state_values2 = torch.zeros(self.batch_size, device=device)
        print(f'next_state_values={next_state_values.shape}')
        print(f'next_state_values2={next_state_values2.shape}')
        xx2 = self.target_net(non_final_next_states)
        print(f'xx2={xx2.shape}')
        xx2out= xx2.max(1)[0].detach()
        next_state_values2[non_final_mask]=xx2out
        print(f'xx2out={xx2out.shape}')
        print(f'next_state_values2={next_state_values2.shape}')
        print(f'next_state_values2[non_final_mask]={next_state_values2[non_final_mask].shape}')
        xx = next_state_values[non_final_mask] = self.target_net(non_final_next_states).max(1)[0].detach()
        print(f'non_final_mask = {non_final_mask.shape}')
        print(f'x={xx.shape}')
        # print(f'next_state_values2={next_state_values2}')
        # print(f'next_state_values2={next_state_values}')
        # print(f'xx2out={xx2out}')
        # print(f'xx={xx}')
        print(f'next_state_values[non_final_mask]={next_state_values[non_final_mask].shape}')
        print(torch.eq(next_state_values2, next_state_values))
        # print(torch.eq(next_state_values2, xx2out))
        # Compute the expected Q values
        expected_state_action_values = (next_state_values * self.gamma) + reward_batch
        print(f'expected_state_action_values={expected_state_action_values.shape}')
        ...
        ################ Double Target Q values####################
        print(".******************************************************This Target double DQN ")
        reshaped_target_selected_q_values_output = torch.zeros(self.batch_size, device=device)
        # print(f'Target_online_q_values={Target_online_q_values.shape}')
        # print(f'Target_online_q_values={Target_online_q_values}')
        # Target_online_q_values[non_final_mask] = self.policy_net(non_final_next_states)
        Target_online_q_values = self.policy_net(non_final_next_states)
        print(f'Target_online_q_values={Target_online_q_values.shape}')
        # print(f'Target_online_q_values={Target_online_q_values[non_final_mask].shape}')
        Target_online_best_q_indices= Target_online_q_values.argmax(dim=1,keepdim=True)
        print(f'Target_online_best_q_indices ={Target_online_best_q_indices.shape}')
        # Target1[non_final_mask] = Target_online_best_q_indices
        # print(f'Target1 = {Target1.shape}')
        # print(f'Target1[non_final_mask] = {Target1[non_final_mask].shape}')
        target_target_double_q_values = self.target_net(non_final_next_states)
        print(f'target_target_double_q_values ={target_target_double_q_values.shape}')
        target_selected_q_values = torch.gather(input=target_target_double_q_values,dim=1,index=Target_online_best_q_indices)
        print(f'target_selected_q_values ={target_selected_q_values.shape}')
        # Target2[non_final_mask] = target_selected_q_values
        # print(f'Target2 = {Target2.shape}')
        # print(f'Target2[non_final_mask] = {Target2[non_final_mask].shape}')

        # reshaped_target_selected_q_values2= target_selected_q_values.view(512)

        reshaped_target_selected_q_values = target_selected_q_values. view(target_selected_q_values.size(0))
        print(f'reshaped_target_selected_q_values ={reshaped_target_selected_q_values.shape}')
        reshaped_target_selected_q_values_output[non_final_mask] = reshaped_target_selected_q_values
        print(f'reshaped_target_selected_q_values_output = {reshaped_target_selected_q_values_output.shape}')
        print(f'reshaped_target_selected_q_values_output[non_final_mask] = {reshaped_target_selected_q_values_output[non_final_mask].shape}')
        # # Compute the expected Q values
        expected_target_selected_q_values = (reshaped_target_selected_q_values_output * self.gamma) + reward_batch
        print(f'expected_target_selected_q_values = {expected_target_selected_q_values.shape}')
        # expected_target_selected_q_values = (reshaped_target_selected_q_values * self.gamma) + reward_batch

        # # Compute Huber loss
        # lossdoubleQ = F.smooth_l1_loss(state_action_values, expected_target_selected_q_values.unsqueeze(1))
        loss = F.smooth_l1_loss(state_action_values, expected_target_selected_q_values.unsqueeze(1))
        print(f'state_action_values={state_action_values.shape}')
        print(f'expected_target_selected_q_values.unsqueeze(1) = {expected_target_selected_q_values.unsqueeze(1).shape}')
        print(f' loss for double Q={loss}')
        # print(f'state_action_values={state_action_values}')
        # print(f'expected_target_selected_q_values = {expected_target_selected_q_values}')
        # print(f'expected_target_selected_q_values.unsqueeze(1) = {expected_target_selected_q_values.unsqueeze(1)}')
